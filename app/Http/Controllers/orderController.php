...
//////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function sendOrder(Request $request,$id){
        ...
        $devices=Device::where('user_id',$order->user_id)->get();
        foreach($devices as $device){
            $this->send($device->token_device,$title,$body);
        }
        Notification::create([
    ...
    public function deleteOrder(Request $request,$id){
        ...
        Notification::create([
            'user_id'=>1,
            'title'=>'The order is deleted',
            'message'=>$user->firstName.'deleted his Order'
        ]);
    ...
    public function allOrderHistory(Request $request){
        ...
        if($order->count()==0){
            return response()->json([
                'message'=> __('msg.messageIfArntHaveOrderInHistory')
            ]);
        }
        return response()->json([
            'message'=> __('msg.messageIfHaveOrderInHistory'),
            'order'=>$order
        ]);
    }
